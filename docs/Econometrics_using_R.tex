...
\begin{verbatim}
##  [1]  7.4877839 -3.0794848 10.0598761  7.4491501  4.5896173 12.8517069
##  [7] -1.5307234 -0.2387279  2.7263075  0.2409325
\end{verbatim}
...
\begin{verbatim}
##  [1]  0.6462069 -1.3434095  1.1304831  0.6389329  0.1005371  1.6561320
##  [7] -1.0518071 -0.8085488 -0.2502888 -0.7182378
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Fatalities }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{fatal_rate =}\NormalTok{ fatal }\OperatorTok{/}\StringTok{ }\NormalTok{pop }\OperatorTok{*}\StringTok{ }\DecValTok{10000}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(year }\OperatorTok{==}\StringTok{ "1988"}\NormalTok{) ->}\StringTok{ }\NormalTok{data}


\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ data}\OperatorTok{$}\NormalTok{beertax, }
     \DataTypeTok{y =}\NormalTok{ data}\OperatorTok{$}\NormalTok{fatal_rate, }
...
\includegraphics{15_Panel2_Implementation_files/figure-latex/unnamed-chunk-2-1.pdf}
...
\begin{Highlighting}[]
\NormalTok{Fatalities }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{fatal_rate =}\NormalTok{ fatal }\OperatorTok{/}\StringTok{ }\NormalTok{pop }\OperatorTok{*}\StringTok{ }\DecValTok{10000}\NormalTok{) ->}\StringTok{ }\NormalTok{data}

\CommentTok{# OLS }
\NormalTok{result_ols <-}\StringTok{ }\KeywordTok{felm}\NormalTok{( fatal_rate }\OperatorTok{~}\StringTok{ }\NormalTok{beertax  }\OperatorTok{|}\StringTok{ }\DecValTok{0} \OperatorTok{|}\StringTok{ }\DecValTok{0} \OperatorTok{|}\StringTok{ }\DecValTok{0}\NormalTok{, }\DataTypeTok{data =}\NormalTok{ data )}
\KeywordTok{summary}\NormalTok{(result_ols, }\DataTypeTok{robust =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# State FE w.o. CRS}
\NormalTok{result_wo_CRS <-}\StringTok{ }\KeywordTok{felm}\NormalTok{( fatal_rate }\OperatorTok{~}\StringTok{ }\NormalTok{beertax  }\OperatorTok{|}\StringTok{ }\NormalTok{state }\OperatorTok{|}\StringTok{ }\DecValTok{0} \OperatorTok{|}\StringTok{ }\DecValTok{0}\NormalTok{, }\DataTypeTok{data =}\NormalTok{ data )}

\CommentTok{# State FE w. CRS}
\NormalTok{result_w_CRS <-}\StringTok{ }\KeywordTok{felm}\NormalTok{( fatal_rate }\OperatorTok{~}\StringTok{ }\NormalTok{beertax  }\OperatorTok{|}\StringTok{ }\NormalTok{state }\OperatorTok{|}\StringTok{ }\DecValTok{0} \OperatorTok{|}\StringTok{ }\NormalTok{state, }\DataTypeTok{data =}\NormalTok{ data )}

\CommentTok{# Report heteroskedasticity robust standard error and cluster-robust standard errors}
\NormalTok{stargazer}\OperatorTok{::}\KeywordTok{stargazer}\NormalTok{(result_wo_CRS, result_w_CRS,  }\DataTypeTok{type =} \StringTok{"text"}\NormalTok{, }\DataTypeTok{se =} \KeywordTok{list}\NormalTok{(}\KeywordTok{summary}\NormalTok{(result_wo_CRS)}\OperatorTok{$}\NormalTok{rse, }\OtherTok{NULL}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# `result_1$rse` contains heteroskedasticity robust standard error.  Put this into `se` option in `stargazer`.}
\NormalTok{stargazer}\OperatorTok{::}\KeywordTok{stargazer}\NormalTok{(result_}\DecValTok{1}\NormalTok{, }\DataTypeTok{type =} \StringTok{"text"}\NormalTok{, }
                     \DataTypeTok{se =} \KeywordTok{list}\NormalTok{(result_}\DecValTok{1}\OperatorTok{$}\NormalTok{rse ) ) }
\end{Highlighting}
\end{Shaded}
...
## attr(,"formula")
## ~rincome | rprice
## <environment: 0x0000000021a4ebe0>
\end{verbatim}
...
## attr(,"formula")
## ~rincome - 1 | rprice
## <environment: 0x0000000021c774f8>
\end{verbatim}
